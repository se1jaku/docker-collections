...
def rage_decrypt(src: str, dst: str, passphrase: str) -> int:
    ...
    print(f"[resource-manager] rage decrypting {src} ...")
    env = os.environ.copy()
...
class ResourceManager:
    ...
    def __init__(self, directory: str):
        ...
        self.resolve_meta()
        print(f"[resource-manager] initialized at directory: {self.directory}")

    @classmethod
    def download_file(cls, url: str, output_file: Path):
        print(f"[resource-manager] requesting {url} ...")
    ...
    def download(self):
        for res in self.meta["resources"]:
            res_path = self.directory / res["filename"]
            if res_path.exists():
                print(f"[resource-manager] {res['filename']} found, skipped")
            else:
                self.download_file(res["update_config"]["url"], res_path)
                print(f"[resource-manager] {res['filename']} downloaded")

    def update(self):
        for res in self.meta["resources"]:
            if res["update_enabled"]:
                self.download_file(res["update_config"]["url"], self.directory / res["filename"])
                print(f"[resource-manager] {res['filename']} updated")
